...
class Sequencer2D(nn.Module):
    def __init__(
        ...
        self.net.classifier[6] = nn.Linear(in_features=4096, out_features=embed_dims[-1])
        params_to_update = []
        ...
        # for param in self.net.parameters():
        #     param.requires_grad = False
        # num_ftrs = self.net.AuxLogits.fc.in_features # Get the number of features output from CNN layer
        # self.net.AuxLogits.fc = nn.Linear(num_ftrs,  out_features=embed_dims[-1])
        # num_ftrs = self.net.fc.in_features
        #self.net.fc = nn.Linear(num_ftrs, out_features=embed_dims[-1])

        # for param in self.net.parameters():
        #     param.required_grad = False
        #
        # num_ftrt = self.net.fc.in_features

        # self.net.fc = nn.Linear(num_ftrt, out_features=embed_dims[-1])
